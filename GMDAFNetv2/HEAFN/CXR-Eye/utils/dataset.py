...
def nfold_split_dataset(file_path, nFold=5, random_state=1):
    ...
    with open(os.path.join(dir_path,"%d_fold_split.csv"%nFold),'w') as f:
        wr = csv.writer(f)
        wr.writerow(split_list) #将分割的结果写进.csv文件
    return split_dict #返回分割好的字典
...
class EyegazeDataset(Dataset):
    ...
    def __len__(self):
        return len(self.csv_file) #DataFrame的长度

    def get_image(self, idx):
        # -- Query the index location of the required file
        new_path = simplify_path(self.csv_file['path'].iloc[idx])
        image_path = os.path.join(self.path_name, new_path)
        ...
        ###### attributes
        healthy = 'no_finding__chx' #定义健康在csv文件中的名称
    ...
    def __getitem__(self, idx):
        ...
        if self.static_heatmap_path:
            ...
            y_hm = self.heatmap_static_transform(y_hm_pil) #heatmap
