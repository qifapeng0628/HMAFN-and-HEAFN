...
def wrap_plotting(args, images, gcam_mask, masks, indices, labels, model_dir, y_cl, 
                  masks_pred=None, mask_three_branch=None, machine_attention_map=None, seperate_save=False):
    ...
    for index, image_name in enumerate(indices):
        ...
        if args.model_type == 'GMDAF':
            # Branch 1 heatmap
            heatmap_branch1_npy = mask_three_branch[0][index].cpu().numpy()
            ...
            # Branch 2 heatmap
            heatmap_branch2_npy = mask_three_branch[1][index].cpu().numpy()
            ...
            # Branch 3 heatmap
            heatmap_branch3_npy = mask_three_branch[2][index].cpu().numpy()
            heatmap_branch3_npy -= heatmap_branch3_npy.min()
            heatmap_branch3_npy /= heatmap_branch3_npy.max()
            heatmap_branch3_npy = np.multiply(heatmap_branch3_npy, 255.0)
            heatmap_branch3_npy = np.rollaxis(heatmap_branch3_npy, 0, 3)
        ...
        # Machine attention map
        if machine_attention_map is not None:
            machine_attn_npy = machine_attention_map[index].cpu().numpy()
            machine_attn_npy -= machine_attn_npy.min()
            machine_attn_npy /= machine_attn_npy.max()
            machine_attn_npy = np.multiply(machine_attn_npy, 255.0)

        gcam_npy = gcam_mask[index].cpu().numpy()
        cmap = cm.jet_r(gcam_npy[0])[..., :3] * 255.0
        gcam_npy = (cmap.astype(np.float64) + raw_image.astype(np.float64)) / 2
        gcam_npy[:, :, [2, 0]] = gcam_npy[:, :, [0, 2]]

        heatmap_image_npy = cv2.applyColorMap(np.uint8(heatmap_image_npy), cv2.COLORMAP_JET)
        heatmap_image_npy = heatmap_image_npy.astype(np.float64) + raw_image.astype(np.float64)
        ...
        if args.model_type == 'GMDAF':
            ...
            heatmap_branch1_npy = heatmap_branch1_npy.astype(np.float64) + raw_image.astype(np.float64)
            ...
            heatmap_branch2_npy = heatmap_branch2_npy.astype(np.float64) + raw_image.astype(np.float64)
            heatmap_branch2_npy = heatmap_branch2_npy / heatmap_branch2_npy.max() * 255.0
            heatmap_branch2_npy[:, :, [2, 0]] = heatmap_branch2_npy[:, :, [0, 2]]
            
            heatmap_branch3_npy = cv2.applyColorMap(np.uint8(heatmap_branch3_npy), cv2.COLORMAP_JET)
            heatmap_branch3_npy = heatmap_branch3_npy.astype(np.float64) + raw_image.astype(np.float64)
            heatmap_branch3_npy = heatmap_branch3_npy / heatmap_branch3_npy.max() * 255.0
            heatmap_branch3_npy[:, :, [2, 0]] = heatmap_branch3_npy[:, :, [0, 2]]
            
            if machine_attention_map is not None:
                machine_attn_colored = cv2.applyColorMap(np.uint8(machine_attn_npy), cv2.COLORMAP_JET)
                machine_attn_colored = machine_attn_colored.astype(np.float64) + raw_image.astype(np.float64)
                machine_attn_colored = machine_attn_colored / machine_attn_colored.max() * 255.0
                machine_attn_colored[:, :, [2, 0]] = machine_attn_colored[:, :, [0, 2]]
        ...
        if not seperate_save:
            ...
            # First row
            plt0 = ax[0, 0].imshow(np.uint8(raw_image), interpolation='bicubic')
            ax[0, 0].set_title(f'Index:{image_name}\nTruth: {label_name}')
            
            plt1 = ax[0, 1].imshow(np.uint8(heatmap_image_npy), interpolation='bicubic')
            ax[0, 1].set_title('EyeGaze Heatmap')
            fig.colorbar(plt1, ax=ax[0, 1], fraction=0.046, pad=0.04)
            
            if machine_attention_map is not None:
                plt_machine = ax[0, 2].imshow(np.uint8(machine_attn_colored), interpolation='bicubic')
                ax[0, 2].set_title('Machine Attention Map')
                fig.colorbar(plt_machine, ax=ax[0, 2], fraction=0.046, pad=0.04)
            else:
                ax[0, 2].set_title('Machine Attention (N/A)')
            
            # Second row - GradCAM outputs
            if args.model_type == 'GMDAF':
                plt2 = ax[1, 0].imshow(np.uint8(heatmap_branch1_npy), interpolation='bicubic')
                ax[1, 0].set_title("Branch 1 (Image) GradCAM")
                fig.colorbar(plt2, ax=ax[1, 0], fraction=0.046, pad=0.04)
                
                plt3 = ax[1, 1].imshow(np.uint8(heatmap_branch2_npy), interpolation='bicubic')
                ax[1, 1].set_title("Branch 2 (Human Attn) GradCAM")
                fig.colorbar(plt3, ax=ax[1, 1], fraction=0.046, pad=0.04)
                
                plt4 = ax[1, 2].imshow(np.uint8(heatmap_branch3_npy), interpolation='bicubic')
                ax[1, 2].set_title("Branch 3 (Machine Attn) GradCAM")
                fig.colorbar(plt4, ax=ax[1, 2], fraction=0.046, pad=0.04)
                
                fig.savefig(f"{write_dir}/ThreeBranch_{image_name}.png")
        
        else:
            # Separate save logic remains the same
...
def visualize_gcam(args, model, test_dl, gcam, target_layer = 'encoder.blocks.6', model_dir=''):
    for images, labels, idx, y_hm, gaze_img, attributes in tqdm_write(test_dl):
        ...
        if args.model_type == 'GMDAF':
            # Get model output and machine attention map
            y_cl, _, _, _, _, _, _ = model(images, gaze_img, show_visualization=False)
            
            # Also get the machine attention map for visualization
            # We need to expose this from the model
            with torch.no_grad():
                machine_attention_map = model.get_machine_attention_map(images)
        else:
            print('Not implemented yet.')
            exit()

        if len(target_layer) > 1:
            # Compute GradCAM for all three branches
            ...
            _, gcam_out3, one_hot = compute_gradCAM(y_cl, labels, gcam, testing_labels, prob_criterion, target_layer[2])
            
            gcam_mask1 = get_mask(gcam_out1, prob_criterion)
            gcam_mask2 = get_mask(gcam_out2, prob_criterion)
            gcam_mask3 = get_mask(gcam_out3, prob_criterion)
            
            gcam_mask = gcam_mask1 + gcam_mask2 + gcam_mask3
            
            wrap_plotting(args, images, gcam_mask, y_hm, idx, labels, model_dir, 
                         prob_criterion(y_cl), 
                         mask_three_branch=[gcam_mask1, gcam_mask2, gcam_mask3],
                         machine_attention_map=machine_attention_map)
        else:
            print('Not implemented yet.')
            exit()